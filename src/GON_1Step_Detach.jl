...
notebook_name = "GON_1StepDetach_ngf16_nz256" * "_" * dataset_name * string(exp_number)
...
inner_loss(theta, x, z) = mean(sum((decode(theta, z; batch_size = size(x,4)) - x).^2, dims = 3))

# Sum Squared Error Definition
rec_loss(theta, x, z) = sum((x - decode(theta, z; batch_size = size(x,4))).^2) / size(x,4)

function total_mse_loss(d::Data)
    # Returns MSE ans Sum Squared over whole dataset
    mse_loss = 0
    sse_loss = 0
    n_instance = 0
    for x in d
        batch_size_ = size(x,4)
        origin = Param(atype(zeros(1, 1, nz, batch_size_)))
        derivative_origin = @diff inner_loss(theta, x, origin)
        dz = grad(derivative_origin, origin)
        z = -dz
        mse_loss += inner_loss(theta, x, z)*batch_size_
        sse_loss += rec_loss(theta, x, z)*batch_size_
        n_instance += batch_size_
    end
    
    mse_loss = mse_loss/n_instance
    sse_loss = sse_loss/n_instance
    
    return mse_loss, sse_loss
end
...
epoch_mse_loss_trn_, epoch_sse_loss_trn_ = total_mse_loss(dtrn)
epoch_mse_loss_tst_, epoch_sse_loss_tst_ = total_mse_loss(dtst)

push!(trn_loss_list, epoch_mse_loss_trn_)
push!(tst_loss_list, epoch_mse_loss_tst_)
push!(trn_rec_loss_list, epoch_sse_loss_trn_)
push!(tst_rec_loss_list, epoch_sse_loss_tst_)
...
info(logger, ("Epoch : 0"))
info(logger, ("Train Loss : $epoch_mse_loss_trn_"))
info(logger, ("Test Loss : $epoch_mse_loss_tst_"))
info(logger, ("Train Reconstruction Loss : $epoch_sse_loss_trn_"))
info(logger, ("Test Reconstruction Loss : $epoch_sse_loss_tst_ \n"))
...
for epoch in progress(1:n_epochs)
    for (i,x) in enumerate(dtrn)
        ...
        derivative_model = @diff inner_loss(theta, x, z)
    ...
    epoch_mse_loss_trn, epoch_sse_loss_trn = total_mse_loss(dtrn)
    epoch_mse_loss_tst, epoch_sse_loss_tst = total_mse_loss(dtst)
    
    push!(trn_loss_list, epoch_mse_loss_trn)
    push!(tst_loss_list, epoch_mse_loss_tst)
    push!(trn_rec_loss_list, epoch_sse_loss_trn)
    push!(tst_rec_loss_list, epoch_sse_loss_tst)
...
#     println("Test Reconstruction Loss : ", epoch_rec_loss_tst)
    ...
    if epoch % logger_checkpoint == 0
        ...
        info(logger,"Train Loss : $epoch_mse_loss_trn")
        info(logger,"Test Loss : $epoch_mse_loss_tst")
        info(logger,"Train Reconstruction Loss : $epoch_sse_loss_trn")
        info(logger,"Test Reconstruction Loss : $epoch_sse_loss_tst \n")
    end
